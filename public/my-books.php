...
// Fine Calculation Function (Copied from history.php / original)
...
$page_title = "My Current Books"; // MODIFIED TITLE
...
$total_stmt = $conn->prepare("SELECT COUNT(*) AS total FROM issued_books WHERE student_id = ? AND return_date IS NULL");
...
// Fetch only CURRENTLY issued books (MODIFIED QUERY to include LIMIT/OFFSET)
$stmt = $conn->prepare("
    ...
    LIMIT ? OFFSET ?
");
$stmt->bind_param("iii", $student_id, $limit, $offset); // Bind student_id, limit, and offset
$stmt->execute();
...
<div class="container mt-5">
    ...
    <h3 class="text-center mb-4">Books Currently Borrowed</h3> <?php if ($result && $result->num_rows > 0): ?>
            ...
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    <?php while ($row = $result->fetch_assoc()): 
                        // Calculate fine for display (always against current date as return_date is NULL)
                        $fine = calculateFine($row['issue_date']);
                        $is_overdue = $fine > 0;
                    ?>
                        <tr>
                            ...
                            <td>
                                <?php if ($row['due_date'] == '0000-00-00' || empty($row['due_date'])): ?>
                                    N/A
                                <?php else: ?>
                            ...
                            <td>
                                <?php if ($is_overdue): ?>
                                    <span class="badge bg-danger">OVERDUE (Fine: ₹<?= $fine ?>)</span>
                                <?php else: ?>
        ...
        <p class="text-end text-muted mt-3">Please return books by the due date to avoid a fine of ₹10 per day. All fines must be paid via cash.</p>
    <?php else: ?>
        <div class="alert alert-info text-center">You currently have no books borrowed.</div>
    <?php endif; ?>
